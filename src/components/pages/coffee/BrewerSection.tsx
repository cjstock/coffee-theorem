...
const BrewerSection = ({ state, dispatch }: Props) => {
    ...
    { brewerId: state.brewer?.id },
    {
      ...
      onSuccess(data) {
        data &&
          dispatch({
            type: 'LoadBrewer',
            brewer: data as z.infer<typeof brewerModel>,
          });
      },
    }
  );
  const title = 'Brewer';
  const description =
    'Who brewed the coffee when you tried it? Did you brew at home? Did you order a cup of coffee from your favorite local coffee shop?';
  return (
