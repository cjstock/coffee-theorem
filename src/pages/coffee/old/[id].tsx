...
const BeanPage: NextPage = () => {
    ...
    const [mapStartPos, setMapStartPos] = useState<google.maps.LatLngLiteral>({
        lat: 0,
        lng: 0,
    });
    ...
    useEffect(() => {
        if (bean) {
            ...
            handleInputChange(
                "sellerTastingNotes",
                bean.sellerTastingNotes || ""
            );
            handleInputChange(
                "sellerBrewMethods",
                bean.sellerBrewMethods || ""
            );
            handleInputChange(
                "sellerDescription",
                bean.sellerDescription || ""
            );
            ...
            handleInputChange(
                "myAdditionalNotes",
                bean.myAdditionalNotes || ""
            );
            handleInputChange("isFavorite", bean.isFavorite || false);
        }
    }, [bean]);

    useEffect(() => {}, [searchQuery, mapStartPos]);
        ...
        <>
            <Layout>
                    ...
                    <div className="flex flex-col w-full lg:flex-row">
                        <div className="flex-grow p-3 grid card bg-secondary-focus rounded-box place-items-center text-primary">
                            ...
                            <div className="w-full text-white grid md:grid-cols-2 grid-cols-1">
                                ...
                                <Input
                                    ...
                                    capitalized={true}
                                    disabled={!isEditMode}
                                    label_key={"sellerBrewMethods"}
                                    value={beanState.sellerBrewMethods}
                                    onChange={handleInputChange}
                                />
                                <Input
                                    label="Buy Link"
                                    type={"url"}
                                    disabled={!isEditMode}
                                    label_key={"sellerBuyLink"}
                                    value={beanState.sellerBuyLink}
                                    onChange={handleInputChange}
                                />
                                <TextArea
                                    label="Tasting Notes"
                                    capitalized={true}
                                    disabled={!isEditMode}
                                    label_key={"sellerTastingNotes"}
                                    value={beanState.sellerTastingNotes}
                                    onChange={handleInputChange}
                                />
                                <TextArea
                                    label="Description"
                                    disabled={!isEditMode}
                                    label_key={"sellerDescription"}
                                    value={beanState.sellerDescription}
                                    onChange={handleInputChange}
                                />
                            </div>
                        </div>
                        ...
                        <div className="flex-grow p-3 grid card bg-secondary-focus rounded-box">
                            ...
                            <TextArea
                                ...
                                capitalized={true}
                                disabled={!isEditMode}
                                label_key={"myTastingNotes"}
                                value={beanState.myTastingNotes}
                                onChange={handleInputChange}
                            />
                            <TextArea
                                label="Brew Methods"
                                capitalized={true}
                                disabled={!isEditMode}
                                label_key={"myBrewMethods"}
                                value={beanState.myBrewMethods}
                                onChange={handleInputChange}
                            />
                            <TextArea
                                label="Additional Notes"
                                disabled={!isEditMode}
                                label_key={"myAdditionalNotes"}
                                value={beanState.myAdditionalNotes}
                                onChange={handleInputChange}
                            />
                        </div>
                    </div>
                        ...
                        >
                            {searchQuery && (
                                <Places
                                    ...
                                    onLocationChange={setMapStartPos}
                                />
                            )}
                        </Map>
                    </Wrapper>
                ...
                <div className="flex flex-row justify-center w-full h-52">
                    <button
                        className="w-1/2 m-3 btn btn-error"
                        type="button"
